# -*- coding: utf-8 -*-
"""Implementation for the `kz.ml.photo_dataset` module."""
...
def build_coco(rows: list[dict], split: str) -> dict:
    ...
    selected = [
        ...
        if r.get("label") in DETECTION_LABELS and is_training_label(r) and _split(r) == split
    ]
    selected.sort(key=lambda r: (str(r.get("ad_id", "")), int(r.get("position") or 0)))
    ...
    for image_id, row in enumerate(selected, 1):
        ...
        if not path.is_file():
            raise FileNotFoundError(f"Labeled image is missing: {path}")
        ...
        if row.get("label") == "damaged" and not boxes:
            raise ValueError(f"Damaged image has no bounding box: {path}")
...
def main() -> None:
    for split in ("train", "audit"):
        ...
        print(
            f"{split:5}: {len(payload['images'])} images, "
            f"{len(payload['annotations'])} boxes → {out}"
        )
