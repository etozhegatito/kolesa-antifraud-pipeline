...
Три страницы, по трём разным задачам:
...
def _cards():
    ...
    if _cards_html is None:
        ...
        # не трогал. Старый отдельный сервер расходился здесь с полной
        # очередью, что тихо меняло смысл разметки. Теперь точка входа одна.
...
async def save_verdict(request: Request):
    """Вердикт разметчика; единственная HTTP-точка записи в журнал."""
    if PUBLIC_DEMO:
        return JSONResponse({"error": "not found"}, status_code=404)
