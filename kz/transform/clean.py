...
Запуск: python -m kz.transform.clean
...
def apply_hard_rules(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # «б/у, пробег 0» — сокрытие пробега/ошибка ввода, НО только у НЕ-новой
    # машины. У текущего модельного года (age==1) 0 км — это правда «новая
    # со склада», даже если condition криво распарсился как «б/у», а лейбл —
    # «первый взнос» (реальный кейс: Changan X5 Plus 2026, коммент «новая без
    # пробега, гарантия 5 лет»). Подозрителен нулевой пробег только у age>=2.
    flag((df["condition"] == "б/у") & (df["mileage_km"] == 0) & (df["age"] >= 2),
