...
COLLECT_CHAIN = [
    ...
    # Фотографии и признаки из них — часть сбора, а не отдельный ритуал.
    # Картинки лежат на CDN: это другой хост со своим лимитом, поэтому они
    # не конкурируют за квоту основного сайта. Признаки считаются здесь же и
    # только для новых снимков, иначе прогон сети повторялся бы впустую.
    step("Сбор 3 · фотографии", "kz.collect.photo_fetch"),
    step("Сбор 4 · признаки из фото", "kz.ml.photo_features"),
]
