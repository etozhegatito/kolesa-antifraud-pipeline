...
# уверенного вердикта.
LABELS = {
    "damaged": "видно повреждение кузова — обвести рамкой",
...
def save_label(ad_id: str, position, path: str, label: str,
    ...
    if label != "damaged" and box:
        raise ValueError(f"рамка имеет смысл только для «damaged», не для «{label}»")
    if box:
...
def main():
    if "--stats" in sys.argv:
        ...
        if s["parts"]:
            print(f"Разобранных: {s['parts']} — отдельный класс, "
