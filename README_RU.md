...
Бесплатный сервер засыпает без трафика, поэтому первый запрос иногда занимает
около минуты. Следующие запросы выполняются быстро.

---

## Цифры

Данные и модель обновлены 3 сентября 2026.

```
данные      12 639 объявлений; 12 455 строк использованы моделью
            1 490 полезно обогащены (11,8%)
            61 052 URL фотографий, 5 304 из них хэшированы
модель      routed MAPE 21,36%  |  медианная APE 13,81%  |  R²(log) 0,935
            out-of-time MAPE 23,24%
            одна общая модель 21,38%; простой baseline — 30,70%
            routed−base: grouped Δ −0,03 п.п., ДИ [−0,17; +0,13]
разброс     bootstrap SD 0,25 п.п.; 95% ДИ общей MAPE [20,87%; 21,87%]
аномалии    184 кандидата, 111 ручных вердиктов
...
```
до 5 млн ₸    MAPE 29,01%   40,9% выборки и 55,5% всей ошибки
5–10 млн      MAPE 15,30%
10–20 млн     MAPE 15,47%
20 млн+       MAPE 17,85%
```

Возраст сам по себе не делит задачу на «простую/сложную»: 0–5 лет — 16,72%,
6–10 — 15,74%, 11–20 — 18,37%, и только 21+ резко поднимается до 29,62%.
Главное пересечение — **21+ лет и дешевле 5 млн**: 3 518 машин (28,2% строк)
создают 41,1% всей ошибки. Поэтому практический приоритет — признаки
...
специалист для базовых прогнозов ниже 5 млн даёт 21,36% против 21,38% у общей
модели. Доверительный интервал разницы накрывает ноль и на grouped CV, и на
out-of-time: на этом срезе нельзя заявлять значимый выигрыш маршрутизации.
...
его MAPE надо снизить с 29,0% примерно до **20,5%**, чтобы общий стал 18%.
...
медианной ошибке (14,21% против 13,94%). Специалист даёт 13,81%.
...
Публичный репозиторий содержит **код, тесты, схему базы и три обученных
артефакта CatBoost для демо**. Артефакты — производные веса деревьев, а не
просматриваемая копия исходных строк. Репозиторий намеренно не содержит
исходных объявлений, текстов продавцов, фотографий, ручных вердиктов и меток
повреждений, пароля от Postgres и сгенерированных отчётов. Поэтому после
клонирования можно сразу запустить готовую оценку цены, но нельзя восстановить
или просмотреть обучающий датасет.
