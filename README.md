...
clean → explore → label_cards
      → train_price_model → residual_detector → ml_dashboard → ml_report
      → evaluate_detector
```

Порядок задан зависимостями: отчёты читают сохранённые артефакты, поэтому
обучение и калибровка идут раньше. Около минуты, сети не касается.
