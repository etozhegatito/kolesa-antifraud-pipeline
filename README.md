...
Данные обновлены 2 сентября 2026, модель — 1 сентября 2026.

```
данные      12 476 объявлений, 15 911 наблюдений цен
            1 453 полезно обогащены (1 508 строк: 23 недоступны, 32 orphan)
            60 267 URL фотографий, 5 004 из них хэшированы
...
антифрод    183 подозрительных (1,5%), 111 вердиктов
            в 65 случайных контролях фрода нет → верхняя граница 4,6% (95%)
разметка    782 кадра; аудит: 38 из 47 damaged-кадров требуют перепроверки
фото-CV     прошлые метрики отозваны до исправления definition drift; в цену не включён
код         11 500+ строк, 247 офлайн-тестов + 6 с test-БД, CI на GitHub Actions
```
...
python -m pytest tests/ -q    # 247 passed, 6 skipped без test-БД
```
...
После аудита definition drift эти supervised-цифры **отозваны**: 38 из 47
`damaged`-кадров требуют визуальной перепроверки. CV никогда не включался в
