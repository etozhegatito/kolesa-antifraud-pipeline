...
Данные обновлены 1 сентября 2026; модель измерена 31 августа.

```
данные      12 320 объявлений, 15 637 наблюдений цен, 1 488 страниц обогащено
            59 513 URL фотографий, 4 704 из них хэшированы
модель      routed MAPE 21,39%  |  медианная APE 13,85%  |  R²(log) 0,935
            out-of-time MAPE 22,37%
            одна общая модель 21,45%; простой baseline — 30,52%
            routed−base: grouped ДИ [−0,20; +0,11], OOT [−0,55; +0,19] п.п.
разброс     bootstrap SD 0,27 п.п.; 95% ДИ общей MAPE [20,88%; 21,95%]
антифрод    182 подозрительных (1,5%), 111 вердиктов
...
код         11 500+ строк, 236 офлайн-тестов + 6 с test-БД, CI на GitHub Actions
```
...
```
до 5 млн ₸    MAPE 29,17%   41% выборки и почти вся ошибка
5–10 млн      MAPE 15,47%
10–20 млн     MAPE 15,25%
20 млн+       MAPE 17,80%
```

Возраст сам по себе не делит задачу на «простую/сложную»: 0–5 лет — 16,87%,
6–10 — 16,00%, 11–20 — 18,39%, и только 21+ резко поднимается до 29,53%.
Главное пересечение — **21+ лет и дешевле 5 млн**: 3 374 машины (28% строк)
создают 40,8% всей ошибки. Поэтому практический приоритет — признаки
...
специалист для базовых прогнозов ниже 5 млн даёт 21,39% против 21,45% у общей
модели, но парный bootstrap-интервал накрывает ноль. Подгонка кривой одной общей модели упирается
...
медианной ошибке (14,12% против 13,96%). Специалист вернул небольшой разрыв:
13,85%. Чем плотнее данные, тем сильнее baseline. Подробно — в
...
python -m pytest tests/ -q    # 236 passed, 6 skipped без test-БД
```
