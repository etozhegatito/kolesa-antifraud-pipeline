...
Данные и модель обновлены 1 сентября 2026.

```
данные      12 320 объявлений, 15 637 наблюдений цен, 1 508 страниц обогащено
            59 513 URL фотографий, 5 004 из них хэшированы
модель      routed MAPE 21,40%  |  медианная APE 13,96%  |  R²(log) 0,935
            out-of-time MAPE 22,57%
            одна общая модель 21,45%; простой baseline — 30,27%
            routed−base: grouped ДИ [−0,18; +0,10], OOT [−0,88; −0,20] п.п.
разброс     bootstrap SD 0,25 п.п.; 95% ДИ общей MAPE [20,90%; 21,91%]
антифрод    180 подозрительных (1,5%), 111 вердиктов
...
код         11 500+ строк, 237 офлайн-тестов + 6 с test-БД, CI на GitHub Actions
```
...
```
до 5 млн ₸    MAPE 28,90%   41% выборки и почти вся ошибка
5–10 млн      MAPE 15,73%
10–20 млн     MAPE 15,38%
20 млн+       MAPE 17,93%
```

Возраст сам по себе не делит задачу на «простую/сложную»: 0–5 лет — 17,09%,
6–10 — 15,99%, 11–20 — 18,37%, и только 21+ резко поднимается до 29,36%.
Главное пересечение — **21+ лет и дешевле 5 млн**: 3 419 машин (28% строк)
создают 40,6% всей ошибки. Поэтому практический приоритет — признаки
...
специалист для базовых прогнозов ниже 5 млн даёт 21,40% против 21,45% у общей
модели. Grouped-интервал ещё накрывает ноль, но на out-of-time специалист
впервые доказан: −0,53 п.п. [−0,88; −0,20]. Подгонка кривой одной общей модели упирается
в асимптоту около 20%.

Всё дело в дешёвом сегменте: при неизменном качестве остальных сегментов
его MAPE надо снизить с 28,9% примерно до **20,5%**, чтобы общий стал 18%.
...
медианной ошибке (14,29% против 13,97%). Специалист даёт 13,96%.
