...
### Пять команд на каждый день

Всё остальное — отдельные шаги, которые оркестратор вызывает сам. По одному
их запускают только при отладке.

| Команда | Сеть | Когда запускать |
|---|---:|---|
| `python -m kz.ops.pipeline_status` | нет | «что где недозаполнено» — начинать отсюда |
| `python -m kz.report.label_cards --serve` | нет | размечать вердикты (пишет в журнал сразу) |
| `python -m kz.ops.run_all --ml` | нет | «я разметил — пересчитай всё»: пересборка, обучение, отчёты, метрики |
| `python -m kz.ops.run_all --fast` | нет | то же, но без обучения: секунды вместо минуты |
| `python -m kz.ops.run_all --collect` | **да** | собрать данные под суточным лимитом запросов |
| `python -m kz.web` | нет | веб-интерфейс: оценка машины и разметка |

### Почему сбор — это `--collect`, а не полный режим

`--collect` делает свежий листинг, а затем добирает пробелы через `catch_up`,
который считает расход запросов по хостам и встаёт, когда суточная квота
выбрана.

Полный режим (`run_all` без флагов) гонит статусы, обогащение и фото напрямую,
**не сверяясь с лимитом**. Именно такая смесь и привела к временной блокировке
адреса 2026-07-23, поэтому он оставлен только для полноты и печатает
предупреждение.

Точечный добор, когда нужен именно он:

```bash
python -m kz.ops.catch_up --run --values     # только обогащение и средняя цена
python -m kz.ops.run_all --light             # только новый листинг
```

### Что делает `--ml`

Восемь шагов в жёстком порядке, около минуты:

```text
1. clean               пересобрать clean_data (подхватить новые вердикты)
2. explore             EDA, дашборд, очередь разметки
3. label_cards         карточки под свежий список подозрительных
4. train_price_model   обучение, grouped CV, out-of-time, артефакт модели
5. residual_detector   калибровка ценового пола
6. ml_dashboard        графики (читает артефакт модели)
7. ml_report           HTML-отчёт (читает ОБА артефакта)
8. evaluate_detector   precision/recall по журналу вердиктов
```

Порядок не произволен: шаги 6 и 7 читают сохранённые артефакты, поэтому
обучение и калибровка идут раньше, а отчёт — после обоих. Зависимости
закреплены тестами, чтобы перестановка шагов не сломала цепочку молча.
...
Открывается на `http://127.0.0.1:8000`, две страницы:

**Оценка машины.** Характеристики и описание — на выходе справедливая цена,
диапазон вместо одного числа, разбор «почему столько» по вкладам признаков,
позиция цены среди похожих объявлений и замечания к самому объявлению.

**Разметка вердиктов.** Те же карточки, что и в автономном режиме
`label_cards --serve`, только внутри общего приложения: вердикт сохраняется
в журнал сразу.

Формулировки намеренно осторожные. Сервис говорит «дешевле большинства
похожих», а не «продадите за день»: это позиция среди выставленных цен, а
прогноза срока продажи у нас пока нет — истории наблюдений хватает только
на быстрые сделки, медленные в неё ещё не попали.

Приложение слушает только `127.0.0.1`: аутентификации и ограничения частоты
запросов в нём нет, наружу открывать нельзя.

### Куда смотреть результат

| Файл | Что внутри |
|---|---|
| `data/eda/ml_report.html` | отчёт по модели цены + подозрительно дешёвые |
| `data/eda/ml_dashboard.png` | качество, важность признаков, остатки |
| `data/eda/label_cards.html` | карточки для разметки |
| `data/eda/dashboard.png` | обзор данных |
| `data/models/price_model.metadata.json` | метрики, отпечаток кода и данных |

### Полный список шагов

| Команда | Сеть | Что делает |
|---|---:|---|
| `python -m pytest tests/ -q` | нет | регрессионные тесты (`103 passed`) |
| `python -m kz.transform.clean` | нет | пересобирает clean-слой |
| `python -m kz.report.explore` | нет | EDA и очередь разметки |
| `python -m kz.report.label_cards` | нет | карточки (без записи в журнал) |
| `python -m kz.ml.train_price_model` | нет | обучение и валидация модели цены |
| `python -m kz.ml.residual_detector` | нет | калибровка ценового пола |
| `python -m kz.ml.learning_curve` | нет | окупается ли дальнейший сбор данных |
| `python -m kz.report.ml_dashboard` | нет | графики модели |
| `python -m kz.report.ml_report` | нет | HTML-отчёт |
| `python -m kz.ml.predict_price` | нет | пример оценки одной машины |
| `python -m kz.report.evaluate_detector` | нет | метрики антифрода |
| `python -m kz.ml.time_to_sell` | нет | прототип срока продажи |
| `python -m kz.ops.migrate_to_postgres` | нет | импорт CSV в базу |
| `python -m kz.ops.run_all` | **да** | полный цикл со всей сетью |
| `python -m kz.collect.parser` | **да** | сбор листинга |
| `python -m kz.collect.check_status` | **да** | статусы объявлений |
| `python -m kz.collect.enrich` | **да** | обогащение страниц |
| `python -m kz.collect.photo_dedup` | **да** | хэши фотографий (CDN) |
| `python -m kz.collect.backfill_avgprice` | **да** | добор средней цены и бейджа |
