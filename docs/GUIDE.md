...
**Цель: MAPE 18%, медиана 12%.** Сейчас 22.7% и 14.5%.

Раньше здесь стояло 15%, и это было неправильно поставленной целью.
Экстраполяция кривой обучения (см. 4.2) даёт для 15% около 40 000
объявлений — в восемь раз больше нынешнего. Для 18% нужно около 17 000,
это достижимо. Ставить цель, которой не дотянуться, значит гарантировать
себе поражение и не заметить настоящего прогресса.

Все рычаги замерены, не предположены:

| Что сделать | MAPE | Медиана |
|---|---:|---:|
| Сейчас | 22.7% | 14.5% |
| Ограничить машинами моложе 25 лет | 19.1% | 12.9% |
| Ограничить машинами моложе 20 лет | **18.3%** | 12.3% |
| Удвоить объём данных (по кривой обучения) | −2.7 п.п. | |
...
Вывод у всех шести замеров один: модель ограничена **объёмом данных, а не
признаками**. Пока кривая обучения не вышла на плато, новые признаки на
тех же 4 800 строках ничего не дадут — им просто не на чем учиться.

Отсюда главный рычаг: **расширить сбор за пределы Алматы.** Сейчас в
`kz/collect/parser.py` четыре категории, все `almaty`. Kolesa покрывает
весь Казахстан; Астана, Шымкент и Караганда дадут ×3–4 к объёму и заодно
превратят `city` из константы в настоящий признак.
...
python -m pytest tests/ -q                  # 107 тестов
```
...
```
данные            4815 объявлений, 1196 обогащено (25%), 5580 фото скачано
подозрительных    38 (0.8%)
вердиктов         85, из них fraud — 0; контрольная группа 50
                  → доля фрода в базе < 4.6% (правило трёх, 95%)

модель цены       MAPE 22.7%  |  медиана 14.5%  |  R²(log) 0.920
                  out-of-time MAPE 23.1%
                  baseline 36.1% → выигрыш 13.4 п.п.
сегменты          <5М 31.1%   5-10М 17.6%   10-20М 16.6%   20М+ 19.4%

сервис            контейнеризован, работает без базы,
                  публичный режим закрывает разметку и лимитит запросы

цель              MAPE 18%, медиана 12%
план              расширить сбор за пределы Алматы (главный рычаг),
                  −4 п.п. ограничить возраст, целиться в MAE вместо RMSE

не работает       новые табличные признаки, текст, фото — всё проверено
                  замером, всё в пределах шума. Упёрлись в объём данных.

блокер №1         объём: кривая обучения не вышла на плато
```
