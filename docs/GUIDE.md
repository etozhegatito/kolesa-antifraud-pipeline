...
**Цель: MAPE 18%.** Сейчас 21.4%, медиана 14.2%.
...
| состояние по фотографиям | пока не доказано, см. FINDINGS §19-20 |
...
Настоящий блокер — дешёвый сегмент: 40% выборки, MAPE 29.2%, и там цену
...
```
данные            11 682 объявления, 1328 обогащено (11%), 5633 фото скачано
подозрительных    186 (1.6%)
вердиктов         111, валидных 88, из них fraud — 0
                  → доля фрода в базе < 3.4% (правило трёх, 95%)
разметка фото     782 кадра: 47 повреждений с рамками, 8 аварий, 592 целых

модель цены       MAPE 21.4%  |  медиана 14.2%  |  R²(log) 0.935
                  out-of-time MAPE 22.2%
                  baseline 30.0% → выигрыш 8.5 п.п.
                  но по МЕДИАНЕ baseline уже сравнялся: 14.30% против 14.19%
сегменты          <5М 29.2%   5-10М 15.5%   10-20М 15.5%   20М+ 17.9%
...
цель              MAPE 18% через дешёвый сегмент: довести его
                  с 29.2% до 22% и общий станет 18.5%
...
не работает       новые табличные признаки, текст, фото — всё проверено
                  замером, всё в пределах шума. Объём тоже исчерпан:
