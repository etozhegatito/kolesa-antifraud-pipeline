...
| Grouped 5-fold R²(log) | **0.936** | 0.935 | 0.853 |
| Grouped median APE | **13.90%** | 14.19% | 14.30% |
| Out-of-time MAPE, 2 300 новых строк | **21.99%** | 22.21% | 31.76% |
...
21.16% против медианной APE 13.90%. Среднее тянет вверх хвост крупных
промахов из дешёвого сегмента.

**Ограничение, которое нужно знать при использовании артефакта.** По
медианной ошибке baseline почти сравнялся с общей моделью: 14.30% против
14.19%. Специалист улучшил медиану до 13.90%, но разрыв всё ещё невелик.
Выигрыш routed-модели в 8.8 п.п. по среднему набирается на хвостах —
редкие комплектации, повреждённые машины, нетипичные пробеги. Если задача
сводится к массовым моделям, модель здесь избыточна.

MAPE модели по цене:

- до 5 млн ₸: 28.6% (4 641 строка);
- 5–10 млн ₸: 15.3% (2 674);
