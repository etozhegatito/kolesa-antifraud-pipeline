...
| размечено кадров фотографий | 782 |

Вердиктов 111, из них валидных 88, и все — «честное объявление». Фрода не
нашлось ни одного, и это результат, а не пустота: по правилу трёх доля
обмана в базе **не выше 3,4%** с доверием 95%. Подробнее — в разделе
...
| Grouped 5-fold R² на `log(price)` | 0,935 | 0,853 |
| Out-of-time MAPE | 22,2% | 31,8% |
| Медианная APE | 14,2% | 14,3% |

Последняя строка — главная новость замера и плохая. **По медианной ошибке
простой справочник сравнялся с моделью.** Выигрыш в 8,5 п.п. по среднему
...
- **средняя ошибка и типичная — разные вещи.** MAPE 21,4% против медианной
  APE 14,2%: половина машин оценивается точнее 14,2%, а среднее тянет вверх
  хвост крупных промахов;
- этот хвост сидит в дешёвом сегменте. До 5 млн — 40% выборки и 29% ошибки,
...
- MAPE 21,4% не следует называть «точностью 78,6%».
...
Где 18% действительно лежат — в дешёвом сегменте, а не в объёме:
...
покрытии. Остаток должен прийти от состояния машины на фотографиях, и там
пока только отрицательные результаты — см. FINDINGS §19-20.
